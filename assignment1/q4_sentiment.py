...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    sentVector = wordVectors[indices, :].sum(axis=0)
...
def getRegularizationValues():
    ...
    values = [ 0, 1e-7, 1e-6, 1e-5, 5e-4, 1e-4, 5e-3, 1e-3, 5e-2, 1e-2, 1e-1, 
               1, 10, 1e2, 5e2, 1e3, 5e3, 1e4, 5e4, 1e5, 1e6, 1e7, 1e8 ]
...
def chooseBestModel(results):
    ...
    bestResult = None
    bestTestAcc = float("-inf")

    for result in results:
        if result['test'] > bestTestAcc:
            bestTestAcc = result['test']
            bestResult = result
